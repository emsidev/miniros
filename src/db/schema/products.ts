...
export const productCategories = pgTable("product_categories", {
  ...
  businessIdx: index("product_categories_business_id_idx").on(table.businessId),
  businessNameUnique: uniqueIndex("product_categories_business_name_unique").on(table.businessId, table.name)
}));
...
export const products = pgTable("products", {
  ...
  requiresRecipeDeduction: boolean("requires_recipe_deduction").default(false).notNull(),
  ...
  businessSkuUnique: uniqueIndex("products_business_sku_unique").on(table.businessId, table.sku)
}));
