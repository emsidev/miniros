...
import { inventoryAdjustmentStatusEnum, requestStatusEnum, saleChangeRequestTypeEnum } from "./enums";

export const saleChangeRequests = pgTable("sale_change_requests", {
  ...
  requestedBy: uuid("requested_by").references(() => employees.id, { onDelete: "set null" }),
...
export const cashDeductions = pgTable("cash_deductions", {
  id: uuid("id").primaryKey(),
  businessId: uuid("business_id").notNull().references(() => businesses.id, { onDelete: "cascade" }),
...
export const inventoryAdjustments = pgTable("inventory_adjustments", {
  ...
  reason: text("reason").notNull(),
  status: inventoryAdjustmentStatusEnum("status").default("pending").notNull(),
  requestedBy: uuid("requested_by").references(() => employees.id, { onDelete: "set null" }),
  reviewedBy: uuid("reviewed_by").references(() => employees.id, { onDelete: "set null" }),
  reviewedAt: timestamp("reviewed_at", { withTimezone: true }),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
