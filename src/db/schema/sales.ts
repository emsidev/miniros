import { bigint, index, pgTable, text, timestamp, uniqueIndex, uuid, numeric } from "drizzle-orm/pg-core";
import { authUsers } from "./auth";
import { businesses } from "./business";
import { employees } from "./employees";
import { fileTypeEnum, paymentMethodEnum, paymentStatusEnum, saleStatusEnum } from "./enums";
...
export const files = pgTable("files", {
  ...
  objectUnique: uniqueIndex("files_bucket_object_unique").on(table.bucketId, table.objectPath)
}));
...
export const sales = pgTable("sales", {
  ...
  changeCents: bigint("change_cents", { mode: "number" }).default(0).notNull(),
  soldBy: uuid("sold_by").references(() => employees.id, { onDelete: "set null" }),
  ...
  businessSaleNumberUnique: uniqueIndex("sales_business_sale_number_unique").on(table.businessId, table.saleNumber),
  businessClientIdUnique: uniqueIndex("sales_business_client_generated_id_unique").on(table.businessId, table.clientGeneratedId)
}));
...
export const payments = pgTable("payments", {
  ...
  businessClientIdUnique: uniqueIndex("payments_business_client_generated_id_unique").on(table.businessId, table.clientGeneratedId)
}));
