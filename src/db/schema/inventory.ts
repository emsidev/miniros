...
export const inventoryItems = pgTable("inventory_items", {
  ...
  businessSkuUnique: uniqueIndex("inventory_items_business_sku_unique").on(table.businessId, table.sku)
}));

export const productRecipeItems = pgTable("product_recipe_items", {
  id: uuid("id").primaryKey(),
  businessId: uuid("business_id").notNull().references(() => businesses.id, { onDelete: "cascade" }),
  ...
  productIdx: index("product_recipe_items_product_id_idx").on(table.productId),
  productItemUnique: uniqueIndex("product_recipe_items_product_item_unique").on(table.productId, table.inventoryItemId)
}));

export const inventoryLocations = pgTable("inventory_locations", {
  id: uuid("id").primaryKey(),
  businessId: uuid("business_id").notNull().references(() => businesses.id, { onDelete: "cascade" }),
  sellingLocationId: uuid("selling_location_id").references(() => sellingLocations.id, { onDelete: "cascade" }),
  ...
  businessIdx: index("inventory_locations_business_id_idx").on(table.businessId),
...
export const inventoryEvents = pgTable("inventory_events", {
  ...
  businessClientIdUnique: uniqueIndex("inventory_events_business_client_generated_id_unique").on(table.businessId, table.clientGeneratedId)
}));

export const inventoryEventLines = pgTable("inventory_event_lines", {
  id: uuid("id").primaryKey(),
  businessId: uuid("business_id").notNull().references(() => businesses.id, { onDelete: "cascade" }),
  eventId: uuid("event_id").notNull().references(() => inventoryEvents.id, { onDelete: "cascade" }),
  inventoryItemId: uuid("inventory_item_id").notNull().references(() => inventoryItems.id, { onDelete: "restrict" }),
...
export const inventoryBalances = pgTable("inventory_balances", {
  id: uuid("id").primaryKey(),
  businessId: uuid("business_id").notNull().references(() => businesses.id, { onDelete: "cascade" }),
  inventoryLocationId: uuid("inventory_location_id").notNull().references(() => inventoryLocations.id, { onDelete: "cascade" }),
  inventoryItemId: uuid("inventory_item_id").notNull().references(() => inventoryItems.id, { onDelete: "restrict" }),
  quantityOnHand: numeric("quantity_on_hand", { precision: 14, scale: 3 }).default("0").notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull()
}, (table) => ({
  locationItemIdx: index("inventory_balances_location_item_idx").on(table.inventoryLocationId, table.inventoryItemId),
  locationItemUnique: uniqueIndex("inventory_balances_business_location_item_unique").on(table.businessId, table.inventoryLocationId, table.inventoryItemId)
}));

export const shiftInventoryCounts = pgTable("shift_inventory_counts", {
  id: uuid("id").primaryKey(),
  businessId: uuid("business_id").notNull().references(() => businesses.id, { onDelete: "cascade" }),
  ...
  shiftItemTypeUnique: uniqueIndex("shift_inventory_counts_shift_item_type_unique").on(table.shiftId, table.inventoryItemId, table.countType)
}));
