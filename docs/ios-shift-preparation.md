...
- Web lint, web/e2e typechecks and production build passed. The repository pre-push hook passed all **33 package tasks**, including **247 web tests with no skips**, after installing PostgreSQL **17.11** through Homebrew and bootstrapping a fresh loopback-only disposable database on port 55432 with the existing fixture bootstrap and migrations. `SHIFT_TEST_DATABASE_URL=postgres://127.0.0.1:55432/miniros_ep00_disposable` was supplied to the hook. The earlier native-database failures were resolved; no hook or test was bypassed.
...
The physical iPhone was available through iPhone Mirroring. Its existing Safari tab at `miniros-web.vercel.app` displayed “Something went wrong — We could not load this workspace. Check your connection and try again.” Launching the existing MINIROS Home Screen app displayed the public landing page and Sign in. These are separate storage/auth containers; neither observation establishes the reported readiness failure. No website data was cleared, no shift was reserved and no update was activated. Worker/controller versions, reply timings and cache errors still require Web Inspector. Phone tap automation returned `noWindowsAvailable`; keyboard Spotlight navigation worked. Mac Safari developer features were enabled successfully after a temporary automation timeout. Both Develop and Apps and Devices Inspection showed only the Mac, with no inspectable iPhone. A USB connection and enabled iPhone Web Inspector are still required. The original phone OS version remains unverified.

Native Firefox **155.0.1** used a fresh loopback test origin on port 4324. The actual production worker verified build `miniros-sAa7fTzX5YScYzLl-Fs8w`, contract 2, repaired an evicted asset and reverified. The real local preparation code committed a disposable simulated session and reached Opening count even with optional persistent-storage permission denied. After entering 10 cups and ₱500 cash, Firefox Work Offline plus a new tab cold launch restored the saved review draft and displayed Offline. Firefox then confirmed the opening counts offline, recorded one ₱100 sale (stock 10→9), saved a −1 stock request (9→8), and saved an eight-cup/₱600 closeout with zero differences. The initial local fixture returned an empty closeout result on reconnection, which broke the final-money display; the fixture now returns calculated, typed reconciliation fields. The rerun browser matrix passed all 18 cases, including the actual final reconciled result screen. Native UI reconnection after this fixture correction is still pending; fixture API evidence is separate from hosted provider and physical iOS acceptance. Firefox Work Offline was disabled at the end.

### Secure repair preview

Commit `24a4939` on `codex/repair-ios-shift-preparation` built successfully as a **Preview** in 1m32s:
[Open the repair preview](https://miniros-4zk5g1em2-mcagbanlog2002-9530s-projects.vercel.app/).
Production was not changed. The phone Safari tab reached Vercel sign-in because the preview retains deployment protection; mirrored tap automation could not complete sign-in. The Vercel connector did not expose this project, but the existing authenticated Brave dashboard and Git integration were available. Do not disable deployment protection to work around this. Sign in to Vercel on the phone, connect USB and enable Web Inspector, then inspect the original production Safari/Home Screen containers before testing disposable shifts on the preview.
